    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-extrabold text-slate-800 tracking-tight">Tessera OS</h1>
        </div>
        <div className="flex items-center gap-4">
          <div className="text-sm font-medium text-slate-500">Tenant Workspace</div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-8 flex flex-col gap-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
            <span className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Active Workspaces</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1">
          ...
          <section className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {/* Org Chart Card */}
              <Link href="/org-chart" className="group bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-400 transition-all cursor-pointer flex flex-col h-full">
                <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  <Network size={24} />
                </div>
                <h4 className="text-lg font-bold text-slate-800 mb-1">Org Chart Builder</h4>
              ...
              {/* Chat Workspace Card */}
              <Link href="/workspace" className="group bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:indigo-400 transition-all cursor-pointer flex flex-col h-full">
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 flex flex-col h-full opacity-70">
                ...
                <p className="text-sm text-slate-500 flex-1">Connect your secure API keys for OpenAI, Gemini, Slack, Jira, and internal databases.</p>
                <div className="mt-4 flex items-center text-slate-400 font-semibold text-sm">
                  Coming Soon
                </div>
              </div>
          ...
          <section className="flex flex-col gap-4">
            <h3 className="text-lg font-bold text-slate-800">Recent Workspaces</h3>
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-col gap-2">
              {isLoading ? (
                <div className="text-sm text-slate-500 text-center py-8 animate-pulse">Loading workspaces...</div>
              ) : recentCharts.length > 0 ? (
                recentCharts.map((chart) => (
                  <Link href="/org-chart" key={chart.id} className="p-3 hover:bg-slate-50 rounded-xl border border-transparent hover:border-slate-200 transition-colors flex items-center justify-between group">
                ...
                <div className="text-sm text-slate-500 text-center py-8">No workspaces found.</div>
              )}
              
              <Link href="/org-chart" className="mt-2 flex items-center justify-center gap-2 w-full py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 font-semibold text-sm rounded-xl border border-slate-200 transition-colors">
