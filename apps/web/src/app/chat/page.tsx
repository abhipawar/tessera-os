...
export default function ChatDashboard() {
  ...
  const runTenantAgent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // The backend returns a single string currently. If we want to extract the name, 
      // we can rely on the compiler's "**[AgentName]**: content" format, or just display it.
      // Our Python compiler formats it as: **[Agent_Name]**: Response
      
      let rawContent = data.result || "Error: No result returned.";
      let agentName = "Agent";

      // Simple regex to extract the name if the compiler injected it
      const nameMatch = rawContent.match(/^\*\*\[(.*?)\]\*\*:\s*/);
      if (nameMatch) {
        agentName = nameMatch[1];
        rawContent = rawContent.replace(/^\*\*\[.*?\]\*\*:\s*/, ''); // Remove the tag from the message body
      }
    ...
    <div className="flex flex-col h-full bg-zinc-950 text-zinc-100 w-full overflow-hidden">
      ...
      <header className="h-16 border-b border-zinc-800 bg-zinc-950 flex justify-between items-center px-6 shrink-0 z-10">
        <div className="flex items-center gap-6">
          <h1 className="text-xl font-bold text-zinc-100">Co-Pilot Chat</h1>
